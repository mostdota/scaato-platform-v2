    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          <input {...register('password')} type={showPass ? 'text' : 'password'} className="input pr-12" placeholder="••••••••" autoComplete="current-password" />
          <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#555] hover:text-[#888]">
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md relative z-10">
        <div className="card p-8">
          ...
          <Suspense fallback={<div className="h-48 flex items-center justify-center"><Loader2 className="w-6 h-6 animate-spin text-[#1DB954]" /></div>}>
            <LoginForm />
          </Suspense>
          <div className="mt-6 text-center text-sm text-[#555]">
            Não tem conta?{' '}
            <Link href="/cadastro" className="text-[#1DB954] hover:text-[#22d460] font-semibold">Cadastre-se</Link>
          </div>
        </div>
